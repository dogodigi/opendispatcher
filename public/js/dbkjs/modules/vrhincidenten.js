...
dbkjs.modules.vrhincidenten = {
  ...
  checkAlleIncidentenRead: function() {
    ...
    if (read) {
      $($("#btn_openvrhincidenten").css({
        color: 'black'
      }).children()[0]).removeClass("fa-exclamation").addClass("fa-fire");
  ...
  nieuweIncidentenLijst: function(incidentIds) {
    ...
    if (incidentListHidden && !me.exclamationFlashInterval) {
      ...
      if (newIncident) {
        $($("#btn_openvrhincidenten").css({
          color: 'red'
        }).children()[0]).removeClass("fa-fire").addClass("fa-exclamation");
  ...
  onIncidentPopupClose: function() {
    ...
    this.incidentPopup.getView().parent().css({
      width: "0%"
    });
    $(".main-button-group").css({
      right: "0%"
  ...
  initVrhService: function() {
    ...
    if (dbkjs.options.vrhDbkUrl && me.token) {
      me.dbkLayer = new OpenLayers.Layer.ArcGIS93Rest("DBK", dbkjs.options.vrhDbkUrl, {
        transparent: "true",
        layers: "hide:0,25",
        token: me.token
      }, {
      ...
      me.dbkLayer2 = new OpenLayers.Layer.ArcGIS93Rest("DBK2", dbkjs.options.vrhDbkUrl2, {
        transparent: "true",
        token: me.token
      }, {
        maxResolution: 0.42
            ...
            $.when(dfdArchiefIncidenten).done(function(renderArchief) {
              ...
              $.each(incidenten, function(i, incident) {
                ...
                marker.events.register("click", marker, function() {
                  me.markerClick(marker, incident);
                });
    ...
    $.ajax({
      ...
      .fail(function(jqXHR, textStatus, errorThrown) {
        def.resolve(function() {
          ...
          $("#incidentenArchief").text("Fout bij ophalen incidenten: " + jqXHR.statusText);
        });
      })
      .done(function(data, textStatus, jqXHR) {
        if (data.error) {
          def.resolve(function() {
            $("#incidentenArchief").empty();
            $("#incidentenArchief").text("ArcGIS service foutmelding " + data.error.code + ": " + data.error.message);
          });
          return;
        }
  ...
  incidentClick: function(incident, setCenter) {
    ...
    $.when(dfdKladblok, dfdInzetEenheden, dfdClassificaties, dfdKarakteristiek).done(function(kladblok, inzetEenheden, classificaties, karakteristiek) {
      ...
      var columns = [{
        property: 'DTG_START_INCIDENT',
        date: true,
        label: 'Start incident'
      }, {
        property: 'T_GUI_LOCATIE',
        date: false,
        label: 'Adres'
      }, {
        property: 'POSTCODE',
        ...
        property: 'PLAATS_NAAM',
        date: false,
        label: 'Woonplaats'
      }, {
        property: 'PRIORITEIT_INCIDENT_BRANDWEER',
        date: false,
        label: 'Prioriteit',
        separate: true
      }];
      ...
      dbkjs.map.updateSize();
      me.incidentPopup.getView().parent().css({
        width: "45%"
      });
      $(".main-button-group").css({
        right: "45%"
